 ...
 */
//todo: should be renamed to NativeSocket.
public final class Socket implements AutoCloseable {
    ...
    public static Socket openTcpIpv4Socket() {
        int family = AF_INET;
    ...
    private static native int setBlocking(int fd);
    ...
    public Socket(int fd, int addressFamily) {
        this.fd = fd;
    ...
    public void setBlocking() {
        int res = setBlocking(fd);
        if (res < 0) {
            throw new UncheckedIOException(new IOException("Failed to setBLocking " + Linux.strerror(-res)));
        }
    }
